...
- 当前已有可复核离线基线，但规模仍小：19 个主要答案召回样本 Recall@5=0.5789，8 个负样本 FalsePositiveRate=1.0；它能暴露具体失败，不能代表大规模线上质量。

优先级较高的后续方向是：围绕当前漏召回实体补强召回与结果聚合，为无关问题增加相关性门控，补齐消息级来源标识，并记录每轮候选、过滤原因和最终注入内容；再扩展信息更新、矛盾事实和用户纠正样本。完整基线和失败分析见 [评测体系](EVALUATION.md)。
...
我会先基于现有评测修复两个已量化问题：主要答案 Recall@5 只有 0.5789，以及负样本 FalsePositiveRate=1.0。具体会记录候选、分数、过滤原因和注入结果，针对漏召回实体优化陈述检索、一跳扩展和聚合，并用相关性阈值降低无关注入。随后扩展信息更新、矛盾事实和用户纠正数据，再补消息级来源、Outbox/幂等恢复和多实例缓存失效。这样每一步都有同一套 gold 回归，而不是继续增加看起来复杂的图谱节点。
