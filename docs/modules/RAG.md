...
仓库提供了运行入口和报告生成逻辑，但评测结果目录默认不进入 Git，相关说明中的成绩也仍是占位符。没有保存且可复核的实际报告前，面试时不能声称已经取得某个基准分数或提升百分比。
...
8. **评测证据尚未固化。** 自建数据集规模有限，公开基准虽有适配器，但仓库没有已提交的结果报告；针对解析、切片和过滤边界的自动化单元测试也仍需补充。
...
**参考回答：** 不能只展示一个成功案例。项目先用固定测试用户、10 份文档和 22 个问题，对比向量、BM25、混合和混合加 Rerank 四种模式，计算 Recall@5、Precision@5、MRR 和 nDCG@5；同时提供 C-MTEB T2Retrieval 和 HotpotQA 适配器，用于中文检索及多跳问答的标准化对比。当前仓库还没有固化的真实运行报告，所以我只会介绍评测设计，不会虚构成绩；下一步是固定模型、参数和数据版本，跑出可复核基线。
